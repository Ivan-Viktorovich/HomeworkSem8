...
# 3 - поиск по фамилии
# 4 - выход
# 5 - изменить информацию о контакте
# 6 - удалить данные
# ----------------------------------------------------------------------------------------
# ДЗ:Задача 38: Дополнить телефонный справочник возможностью изменения и удаления данных. 
...
def transformation(file_name, stroka):
    a = 0
    data = open(file_name, 'r')
    for line in data:
        if stroka in line:
            add_cont (stroka, input("Введите новые данные"))
            a = 123
            break
    if a != 123:
        print("нет такого")
